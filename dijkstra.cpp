...
public:
  ...
  void setSource(const char &newSource) {
    ...
    std::cerr << CError << "Error: There is no element " << newSource << ColorOff << std::endl;
  }
  ...
  void addEdge(const char &from, const char &to, int distance) {
    if (nodes.count(from) && nodes.count(to)) {
      ...
      std::cerr << CError << "Error: Node(s) not found in the graph." << ColorOff << std::endl;
    }
  }

  void dijkstra() {
    if (source == nullptr) {
      std::cerr << CError << "Error: Source node not set." << ColorOff << std::endl;
      return;
    }
    if (nodes.size() <= 1) {
      std::cerr << CError << "Error: There are either no nodes or only one node" << ColorOff << std::endl;
      return;
    }
  ...
  void printPath() {
    if (path.empty()) { // Check if shortest paths have been calculated
      std::cerr << CError << "Error: Shortest paths have not been calculated. Run Dijkstra's "
                   "algorithm first."
                << ColorOff << std::endl;
      return;
    }
